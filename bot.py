...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        "Привет! Я бот-тест по истории Серпухова 🦚\n\n"
        "Команды:\n"
        "/test — начать тест\n"
        "/score — свои результаты\n"
...
async def cmd_test(message: types.Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("INSERT INTO attempts (user_id, started_at) VALUES (?, ?)", (user_db_id, started_at))
        await db.commit()
        attempt_id = cur.lastrowid
        for i in range(NUM_QUESTIONS_PER_TEST):
            await db.execute("INSERT INTO answers (attempt_id, q_index, chosen, correct) VALUES (?, ?, ?, ?)",
                             (attempt_id, i, -1, -1))
        await db.commit()

    user_sessions[user.id] = {"attempt_id": attempt_id, "questions": questions, "current_q": 0}
...
async def send_question(user_id: int, attempt_id: int, q_index: int):
    ...
    if not session or session["attempt_id"] != attempt_id:
        await bot.send_message(user_id, "⏳ Сессия устарела. /test чтобы начать заново")
        return
    ...
    for i, option in enumerate(q["options"]):
        kb.add(InlineKeyboardButton(option, callback_data=f"answer|{attempt_id}|{q_index}|{i}"))
...
async def process_answer(callback_query: types.CallbackQuery):
    try:
        ...
        attempt_id, q_index, choice = int(attempt_id_s), int(q_index_s), int(choice_s)
    except Exception:
    ...
    session = user_sessions.get(user_id)
    if not session or session["attempt_id"] != attempt_id:
        await callback_query.answer("⏳ Сессия устарела. /test чтобы начать заново", show_alert=True)
        return
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE answers SET chosen=?, correct=? WHERE attempt_id=? AND q_index=?",
    ...
    await bot.send_message(user_id, f"Вы выбрали: {q['options'][choice]}\nПравильный ответ: {q['correct_answer']}\n{explanation}")
    ...
    await bot.send_message(user_id, f"Текущий счёт: {score}/{NUM_QUESTIONS_PER_TEST}")
    session["current_q"] += 1
...
async def cmd_score(message: types.Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT score, started_at FROM attempts WHERE user_id=? ORDER BY id DESC LIMIT 10", (user_db_id,))
        rows = await cur.fetchall()
    if not rows:
        await message.reply("У тебя ещё нет результатов. /test чтобы начать.")
        return
    text = "📊 Твои последние попытки:\n" + "\n".join(f"{i+1}. {score}/{NUM_QUESTIONS_PER_TEST} — {started}" for i, (score, started) in enumerate(rows))
...
async def show_leaderboard(chat_id: int):
    ...
    if not rows:
        ...
        return
    text = "🏆 Топ пользователей:\n" + "\n".join(f"{i+1}. {u or f or 'Пользователь'} — {s}/{NUM_QUESTIONS_PER_TEST}" for i, (u, f, s) in enumerate(rows))
